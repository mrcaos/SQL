...
select COUNT(*) 
from productos where precio > 7000;

-- CALCULAR EL PROMEDIO DE PRODUCTOS QUE TIENEN STOCK
select --avg(precio) 
from productos where en_stock = true;
...
select nombre_producto, MAX(precio) from productos group by categoria; -- no es un dato que permite agrupacion
